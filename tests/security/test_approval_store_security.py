...
def test_invalid_effect_digest_and_time_window_are_rejected(tmp_path: Path) -> None:
    ...
    for effect, digest, expiry in (
        ...
        with pytest.raises(ApprovalError):
            store.request(
                approval_id=uuid4(), project_id="p", run_id=RunId.new(),
                action_digest=digest, effect=effect, requested_by="runtime",
...
def test_only_pending_records_can_be_decided(tmp_path: Path) -> None:
    ...
    record = store.request(
        approval_id=uuid4(), project_id="p", run_id=RunId.new(),
        action_digest="sha256:" + "a" * 64, effect=ActionEffect.SENSITIVE,
        requested_by="runtime", requested_at=now, expires_at=now + timedelta(minutes=1),
    )
    store.decide(record.approval_id, status=ApprovalStatus.REVOKED,
