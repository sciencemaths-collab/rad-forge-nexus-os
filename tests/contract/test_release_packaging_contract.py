...
def test_alpha_versions_and_release_commands_are_aligned() -> None:
    ...
    assert project["version"] == "0.2.0a6"
    ...
    assert '__version__ = "0.2.0a6"' in (ROOT / "src/nexus_os/__init__.py").read_text(
        encoding="utf-8"
    )
...
def test_container_is_pinned_non_root_and_installs_only_the_built_wheel() -> None:
    ...
    assert "pip install --no-cache-dir /tmp/nexus_os-0.2.0a6-py3-none-any.whl" in dockerfile
    assert "!dist/nexus_os-0.2.0a6-py3-none-any.whl" in dockerignore
...
def test_quickstart_uses_real_release_channels_and_documents_container_boundary() -> None:
    ...
    assert "gh release download v0.2.0a6" in quickstart
    assert "--dir dist" in quickstart
    assert "sha256sum --check dist/SHA256SUMS" in quickstart
    assert "gh attestation verify dist/nexus_os-0.2.0a6-py3-none-any.whl" in quickstart
    assert "ghcr.io/sciencemaths-collab/rad-agent:v0.2.0a6" in quickstart
