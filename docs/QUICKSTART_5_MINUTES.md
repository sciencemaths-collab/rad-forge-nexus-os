...
evidence archive from the [GitHub release](https://github.com/sciencemaths-collab/rad-forge-nexus-os/releases/tag/v0.2.0a4):

```bash
mkdir -p dist
gh release download v0.2.0a4 --repo sciencemaths-collab/rad-forge-nexus-os --dir dist
sha256sum --check dist/SHA256SUMS --ignore-missing
gh attestation verify dist/nexus_os-0.2.0a4-py3-none-any.whl \
  --repo sciencemaths-collab/rad-forge-nexus-os
pipx install ./dist/nexus_os-0.2.0a4-py3-none-any.whl
rad --version
rad --help
```
...
```bash
docker pull ghcr.io/sciencemaths-collab/rad-agent:v0.2.0a4
docker run --rm ghcr.io/sciencemaths-collab/rad-agent:v0.2.0a4 --version
```
...
docker run --rm -it --network host \
  ...
  ghcr.io/sciencemaths-collab/rad-agent:v0.2.0a4 \
