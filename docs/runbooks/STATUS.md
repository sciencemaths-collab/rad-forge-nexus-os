...
| AF. CI/release evidence | TESTED | 281-test suite, 13 fail-fast gates, secret scan, SBOM/provenance/report bundle pass |
...
Component AF is TESTED, not release-qualified. GitHub-hosted execution remains unobserved in
this local environment, private-repository Dependency Review may require GitHub Advanced
Security, and artifact attestation is not yet generated by a trusted hosted workflow. The
generator intentionally marks clean-room qualification, independent review, owner approval,
and release-candidate status false. Component AG performs final clean-room qualification and
the independent release review; production approval remains owner-gated afterward.
